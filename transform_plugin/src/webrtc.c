     ...
     )
{
 ...
 //      "udpsrc address=%s port=%s  ! doherh264depay ! h264parse ! avdec_h264 ! deinterlace ! timestamp fps=25.0 !  x264enc  ! video/x-h264, profile=baseline !  rtph264pay config-interval=-1 !  "
    ...
    else if ((add_filter == 5))
    {
     ...
     sprintf(pipeline_str, "webrtcbin name=webrtcbin stun-server=stun://" STUN_SERVER " "
       "udpsrc address=%s port=%s  !  application/x-rtp, encoding-name=H264, clock-rate=90000 ! rtph264depay ! h264parse ! video/x-h264, stream-format=avc ! h264parse ! video/x-h264, stream-format=byte-stream ! nvv4l2decoder name=dec nvstreammux name=mux  width=1920 height=1080 batch-size=1 batched-push-timeout=4000 dec.src ! mux.sink_0 mux. ! queue  ! nvinfer config-file-path= objectdetect/pgie_detectnet_v2_tlt_config.txt ! nvvideoconvert ! nvdsosd ! nvvideoconvert ! queue !  nvv4l2h264enc ! rtph264pay config-interval=-1 !  "
       "application/x-rtp,media=video,encoding-name=H264,payload="
       RTP_PAYLOAD_TYPE " ! webrtcbin. ", address, port);
    }
    ...
    else if ((add_filter == 8))
    {
     ...
     {
	     sprintf(pipeline_str, "webrtcbin name=webrtcbin  "
			     "udpsrc address=%s port=%s  multicast-iface=%s  ! mpeg4filter ! mpeg4videoparse  !  timestamp fps=%s ! avdec_mpeg4 skip-frame=5 ! videoconvert ! timestamp fps=%s ! x264enc tune=zerolatency !  video/x-h264, profile=baseline ! rtph264pay config-interval=-1 !  "
			     "application/x-rtp,media=video,encoding-name=H264,payload="
			     RTP_PAYLOAD_TYPE " ! webrtcbin. ", address, port, interface, fps, fps);
     }	     
     else
     {
    ...
    {
     sprintf(pipeline_str, "webrtcbin name=webrtcbin "
       "udpsrc address=%s port=%s  multicast-iface=%s  ! mpeg4filter ! mpeg4videoparse  !  timestamp fps=%s ! avdec_mpeg4 skip-frame=5 ! videoconvert ! videorate ! video/x-raw, framerate=%s/1  ! x264enc tune=zerolatency ! timestamp fps=1000 !  video/x-h264, profile=baseline ! rtph264pay config-interval=-1 !  "
       "application/x-rtp,media=video,encoding-name=H264,payload="
       RTP_PAYLOAD_TYPE " ! webrtcbin. ", address, port, interface, fps, fps);
    }
    else
